...
FEATURES = ["Date" ,"Close", "Volume", "Open", "High", "Low"]
...
def load_and_process_data(company, start_date, end_date, feature_columns=['Close', 'Volume', 'Open', 'High', 'Low'], n_steps=50, nan_strategy='drop', split_method='ratio', test_size=0.2, split_by_date=None, save_local=False, local_path="dataset.csv", scale=True, scaler_type='minmax', shuffle=True):
    """
        Params:
...
## 1. d. This function will have the option to allow you to store the downloaded data on your local machine for future uses and to load the data locally to save time.
    ...
    #make dataframe if not dataframe
...
##1. b. This function will allow you to deal with the NaN issue in the data.
    ...
    #
    date_values = pd.DataFrame(data["Date"])
    ...
    data = data[feature_columns] ##use the feature_columns from dataset
    # save a copy of the dataset after initial cleaning
...
##https://machinelearningmastery.com/standardscaler-and-minmaxscaler-transforms-in-python/
    ...
    if scale:
        ...
        data2 = data.join(date_values)
        data2.to_csv("scaled_data_with_date.csv")
...
def create_model(sequence_length, n_features, neurons =256, dl_network="CNN", n_layers=2, dropout=0.2,
    ...
    return model
# arguments for model
model = create_model(PREDICTION_DAYS, len(FEATURES), loss='huber_loss', neurons=128, dl_network= LSTM, n_layers= 1,
...
model.fit(X_train, y_train, epochs=1, batch_size=20, callbacks=[early_stopping])
...
# print(X_test.shape)
# #(172, 50, 5)
# # reshape so only predicting close price
# print(X_test.dtype)
...
def multistep_prediction(k_days, X_test, model):
    # y is close data value
    forcast =[]
    #single step
    last = X_test[-1].reshape(1, -1, X_test.shape[-1])
    print(last.shape)
    last_prediction = model.predict(last)
    #print(prediction)

    # fabricate prediction days
    forcast.append(last_prediction)
    ...
    # fabricate some dates
    # get date and closing price pairs
    closing = data["Close"]
    dates = data["Date"]
    print(closing.tail)
    print(dates.tail)
    ...
    #laste_date 50,5
    # print(last_date.shape)
    # print(last_date)
    # 1,50,5
    # print(real_data.shape)
    # print(real_data)
    #vector size 2 required


    #X_test[-1] get last value in X_test
    #reshape form 50,5 to 1,50,5 where (sample, timestepss, features)
    # so 1 being 1 sample
    # -1 being the number of timesteps which is figured out by numpy itself. finding the length of the array and remaining dimensions
    # X_test.shape[-1] have numpy figure it out for the features
    
   #print(model.predict(X_test))
    #predict the price k_days into the future
    #print(X_test.shape)
    #print(X_test)
    #print(X_test.dtype)
    #172,50,5
    ...
    # print(real_data.shape)
    # slice for closing price

    #(1,50,5)
    # error at reshape cannoty reshape array of size 250 into shape(1,50,1)
    # real_data = np.reshape(real_data, (real_data.shape[0], real_data.shape[1], 1))

    # real data is the dataset that will be used for the prediciton test input
    # first = model.predict(real_data)
    # forcast.append(first)
    
    # for _ in range(k_days -1):
        # next = first.reshape(1, PREDICTION_DAYS, 5)
    #prediction = scaler.inverse_transform(prediction)
        # print(model.predict(next))
        # add prediction to data for consecutive steps

    # print(f"Prediction {k_days} consecutive days: {forcast}")
...
multistep_prediction(4, X_test, model)


# new funciton b5
def simple_multivariate_prediction(features_list):
    # features list is input for predicting a closing price of the company for a specified day in the future
    pass
